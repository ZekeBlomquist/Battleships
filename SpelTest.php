...
<head>
	...
	<script>
		...
		//Array med kordinater för ens egna skepp
		...
		function placera(x, y) {
			if (!start) {
				...
				if (antalKlick == 1) {
					...
					shipBP = [];
					console.log(egnaSkepp.length);

					//sätter alla rutor till skepp-färgen
					egnaSkepp.push((firstX+"-"+firstY));
		...
		function remove4(){
			...
			if (firstY+-skeppLength > -2) {
				console.log(firstY+skeppLength)
